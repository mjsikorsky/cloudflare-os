...
it('retains native Cloudflare Access admission when the server does not advertise a host', async () => {
  ...
  const methods = { authenticateFromCfAccess: vi.fn<() => RpcStub<AuthenticatedApi>>(() => auth), authenticateExternal: vi.fn<() => RpcStub<AuthenticatedApi>>(), authenticate: vi.fn<(token: string) => RpcStub<AuthenticatedApi>>() }
  ...
  } finally { await React.act(async () => root.unmount()); container.remove(); vi.unstubAllEnvs() }
